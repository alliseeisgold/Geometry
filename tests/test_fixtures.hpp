...
class GeometryTest : public ::testing::Test {
 ...
 protected:
  ...
  static void AreVectorsSimilar(const Vector& v1, const Vector& v2) {
    ...
    ASSERT_DOUBLE_EQ(
        static_cast<double>(v1.GetX()) / static_cast<double>(v2.GetX()),
...
class PointTest : public GeometryTest {
 public:
  ~PointTest() override { delete expected_moved_point_; }

 protected:
  void ConstructTest(int64_t point_x, int64_t point_y, int64_t p1_x,
                     int64_t p1_y, int64_t p2_x, int64_t p2_y,
                     int64_t expected_x, int64_t expected_y) {
...
class SegmentTest : public GeometryTest {
 public:
  ~SegmentTest() override { delete expected_moved_segment_; }

 protected:
  void ConstructTest(int64_t segment_p1_x, int64_t segment_p1_y,
                     int64_t segment_p2_x, int64_t segment_p2_y, int64_t p1_x,
                     int64_t p1_y, int64_t p2_x, int64_t p2_y,
    ...
    p1_ = new Point(p1_x, p1_y);
    p2_ = new Point(p2_x, p2_y);
    expected_moved_segment_ =
        new Segment(Point(segment_expected_p1_x, segment_expected_p1_y),
...
class RayTest : public GeometryTest {
 ...
 protected:
  void ConstructTest(int64_t ray_p1_x, int64_t ray_p1_y, int64_t ray_p2_x,
                     int64_t ray_p2_y, int64_t p1_x, int64_t p1_y, int64_t p2_x,
                     int64_t p2_y, int64_t ray_expected_p_x,
                     int64_t ray_expected_p_y, int64_t ray_expected_v_x,
                     int64_t ray_expected_v_y) {
    shape_ = new Ray(Point(ray_p1_x, ray_p1_y), Point(ray_p2_x, ray_p2_y));
    p1_ = new Point(p1_x, p1_y);
    p2_ = new Point(p2_x, p2_y);
...
class LineTest : public GeometryTest {
 protected:
  void ConstructTest(int64_t line_p1_x, int64_t line_p1_y, int64_t line_p2_x,
                     int64_t line_p2_y, int64_t p1_x, int64_t p1_y,
                     int64_t p2_x, int64_t p2_y, int64_t line_expected_a,
                     int64_t line_expected_b, int64_t line_expected_c) {
    shape_ = new Line(Point(line_p1_x, line_p1_y), Point(line_p2_x, line_p2_y));
    p1_ = new Point(p1_x, p1_y);
    p2_ = new Point(p2_x, p2_y);
  ...
  void TestMove() override {
    ...
    if (line_expected_a_ == 0 || moved_line_a == 0) {
      ...
      a_k = static_cast<double>(moved_line_a) /
            static_cast<double>(line_expected_a_);
    }

    if (line_expected_b_ == 0 || moved_line_b == 0) {
      b_k = 0;
    } else {
      b_k = static_cast<double>(moved_line_b) /
            static_cast<double>(line_expected_b_);
    }

    if (line_expected_c_ == 0 || moved_line_c == 0) {
      c_k = 0;
    } else {
      c_k = static_cast<double>(moved_line_c) /
            static_cast<double>(line_expected_c_);
    }
...
class CircleTest : public GeometryTest {
 public:
  ~CircleTest() override { delete expected_moved_circle_; }

 protected:
  void ConstructTest(int64_t point_x, int64_t point_y, size_t r, int64_t p1_x,
                     int64_t p1_y, int64_t p2_x, int64_t p2_y,
                     int64_t expected_p_x, int64_t expected_p_y,
                     size_t expected_r) {
    shape_ = new Circle(Point(point_x, point_y), r);
    p1_ = new Point(p1_x, p1_y);
    p2_ = new Point(p2_x, p2_y);
    expected_moved_circle_ =
        new Circle(Point(expected_p_x, expected_p_y), expected_r);
  }

  void TestMove() override {
    SetupMoveTest();
    auto moved_circle = dynamic_cast<Circle*>(moved_shape_);
    ASSERT_TRUE(ArePointsEqual(moved_circle->GetCentre(),
                               expected_moved_circle_->GetCentre()));
    ASSERT_DOUBLE_EQ(moved_circle->GetRadius(),
                     expected_moved_circle_->GetRadius());
  }
